# Name: Anton Overby
# Prog Purpose: This program computes grades for students using a Python LIST of data in Python DICTIONARIES (as student records). 
# Each student has 3 graded items: Homework, Midterm exam, and Final exam.
# Each item is worth 100 points
# The student's final grade is the average of the three items.
...
def print_grade_report():
    ...
    for i in range(len(stu)):
        sname = f"{stu[i]['name']:<18}"
        hw = str(stu[i]['hw'])
        midex = str(stu[i]['mid_exam'])
        finex = str(stu[i]['fin_exam'])
        fingrade = f"{stu[i]['fin_grade']:6,.2f}"
